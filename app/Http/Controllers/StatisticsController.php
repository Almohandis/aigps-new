    ...
    public function recoveriesReport($report_by, $names)
    {
        ...
        switch ($report_by) {
            ...
            case 'Hospital':
                ...
                $data = DB::select('SELECT hos1.name, hos1.city, ( SELECT COUNT(*) FROM infections AS inf1 WHERE inf1.hospital_id = hos1.id ) AS total_cases, ( SELECT COUNT(*) FROM infections AS inf1 WHERE inf1.hospital_id = hos1.id AND inf1.is_recovered = 1 ) AS total_recoveries, ( ROUND( ( ( SELECT total_recoveries )/( SELECT COUNT(*) FROM infections AS inf1 WHERE inf1.hospital_id = hos1.id )* 100 ), 2 ) ) AS recovery_pcnt, hos1.capacity FROM hospitals AS hos1;');
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Recovery analysis report by hospital';
    ...
    public function deathsReport($report_by, $names)
    {
        ...
        switch ($report_by) {
            case 'City':
                ...
                $data = json_decode($data);
                // return $data;
                $report_title = 'Death analysis report by city';
            ...
            case 'Hospital':
                ...
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Death analysis report by hospital';
    ...
    public function distributionOfHospitals($report_by, $names)
    {
        ...
        switch ($report_by) {
            case 'City':
                ...
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Distribution of hospitals by city';
    ...
    public function infectionsReport($report_by, $names)
    {
        ...
        switch ($report_by) {
            case 'City':
                ...
                // return $data_by_city;
                $report_title = 'Infection analysis report by city';
                $cities = [
                    'Alexandria'  =>  [
                        'lat' =>  31.2001,
                        'lng' =>  29.9187,
            ...
            case 'Date':
                ...
                break;
            case 'Age segment':
                $data = DB::select('SELECT DISTINCT IF( TIMESTAMPDIFF(YEAR, u1.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u1.birthdate, now())<= 40, "Youth", "Elder" ) ) AS age, ( SELECT COUNT(distinct u2.id) FROM users AS u2 WHERE u2.id in ( select id from infections_id ) AND IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 40, "Youth", "Elder" ) )=( SELECT Age ) ) AS total, ( SELECT COUNT(distinct u2.id) FROM users AS u2 WHERE u2.id in ( select id from infections_id ) AND u2.gender = "Male" AND IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 40, "Youth", "Elder" ) )=( SELECT age ) ) AS male, ROUND( ( SELECT COUNT(distinct u2.id) FROM users AS u2 WHERE u2.id in ( select id from infections_id ) AND u2.gender = "Male" AND IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 40, "Youth", "Elder" ) )=( SELECT age ) )/( SELECT total )* 100, 2 ) AS male_pcnt, ( SELECT COUNT(distinct u2.id) FROM users AS u2 WHERE u2.id in ( select id from infections_id ) AND u2.gender = "Female" AND IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 40, "Youth", "Elder" ) )=( SELECT age ) ) AS female, ROUND( ( SELECT COUNT(distinct u2.id) FROM users AS u2 WHERE u2.id in ( select id from infections_id ) AND u2.gender = "Female" AND IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 20, "Children", IF( TIMESTAMPDIFF(YEAR, u2.birthdate, now())<= 40, "Youth", "Elder" ) )=( SELECT age ) )/( SELECT total )* 100, 2 ) AS female_pcnt FROM users AS u1 group by age ORDER BY age;');
                $data = json_encode($data);
                $data = json_decode($data);
                // return $data;
    ...
    public function distributionOfDoctorsInHospitals($report_by, $names)
    {
        ...
        switch ($report_by) {
            case 'City':
                ...
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Distribution of hospital doctors by city';
            ...
            case 'Hospital':
                ...
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Distribution of hospital doctors by hospital';
    ...
    public function hospitalizationStatus($report_by, $names)
    {
        ...
        switch ($report_by) {
            case 'City':
                ...
                $data = json_encode($data);
                $data = json_decode($data);
                $report_title = 'Hospitalization status by city';
