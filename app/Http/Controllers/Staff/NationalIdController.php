    ...
    public function modify(Request $request)
    {
        ...
        if ($request->add) {
            if ($check_id) {
                ...
                $national_id = new NationalId([
                    'national_id' => $request->entered_id,
                ]);
                $national_id->save();
                return redirect('/nationalid/modify')->with('message', 'National ID added successfully');
            }
        } else if ($request->delete) {
            if ($check_id) {
                $check_id->delete();
                return redirect('/nationalid/modify')->with('message', 'National ID deleted successfully');
            } else {
